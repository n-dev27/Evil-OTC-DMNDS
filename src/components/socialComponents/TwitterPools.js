...
import { useContractRead, useAccount, useNetwork, useBalance } from 'wagmi'
import BuyConfirmationModal from '../buyComponents/BuyConfirmationModal'
...
const TwitterTokenPools = ({ twitterUser, isTwitterAuthenticated }) => {
  const [poolOption, setPoolOption] = useState(false)

  let twitterUserScreenName = twitterUser?.reloadUserInfo?.screenName
  ...
  const [nativeTokenPrice, setNativeTokenPrice] = useState('10000000000000000')
  ...
  const { data: searchedPoolsData } = useContractRead({
    ...
    args: ['4', '0x0000000000000000000000000000000000000000', '0x0000000000000000000000000000000000000000', twitterUserScreenName],
    onSuccess(searchedPoolsData) {
      console.log('Found Twitter Assigned Pools', searchedPoolsData)
    ...
    onError(error) {
      console.log('Error retrieve Twitter pools from contract', error)
    },
  })
      ...
      {twitterUserScreenName && (
        <div className="flex items-center justify-center">
          ...
          <div className="text-blue-400">
            <a href={'https://twitter.com/' + twitterUserScreenName} target="_blank" rel="noopener noreferrer">
              @{twitterUserScreenName}
            </a>
          </div>
        </div>
            ...
            <div className="flex w-full justify-center ">
              <div className="md:text-large w-full text-sm text-[#566C90] dark:text-white">
                {formattedPoolsData && formattedPoolsData?.length > 0 ? (
                        ...
                        <table className="border-separate border-spacing-1">
                          ...
                          <>
                            {formattedPoolsData?.map((pool, index) => {
                              let tokenImage = ReactDOMServer.renderToString(
                                <GetTokenImage token={pool?.tokenAddress} />
                              )
                              return (
                                <tbody key={index} className="text-center">
                                  <tr>
                                    ...
                                    <td>
                                      <div>
                                        {utils.commify(
                                          utils.formatUnits(pool?.poolData[5]?.toString(), 18)
                                        )}
                                      </div>
                                ...
                                <div className="w-full rounded-xl bg-white p-2 dark:bg-gray-500/30">
                                  <div className="w-full flex-col">
                                    ...
                                    <div className="flex justify-between py-1">
                                      <div className="flex-col">
                                        <div className="py-1 text-start">Buy Price</div>
                                        {pool.poolData[4]?.toString() > '0' ? (
                                          <div className="flex items-center">
                                            ...
                                            <div className="flex px-1">
                                              (<a className="text-[.6rem] text-gray-400">~$1,519 USD</a>)
                                            </div>
                                          </div>
                                        ) : (
                                          <div className="flex items-center">
                  ...
                  <div>
                    <div
                      className="mb-2 flex justify-center rounded-lg bg-red-500 p-2 text-center text-sm text-white "
                      role="alert"
                    >
                      <span className="font-medium md:text-xl">
                        No pools currently offered to authenticated Twitter account ({twitterUserScreenName}).
                      </span>
                    </div>
                  </div>
                )}
...
export default TwitterTokenPools
