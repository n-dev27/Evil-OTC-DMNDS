...
function TxHistoryBuyContainer (props) {
  ...
  return (
    <>
    {isConnected ?
      <div className={`${buyLoading ? 'h-full' : 'h-full sm:h-fit'} md:text-large text-sm w-full flex flex-col justify-center items-center text-center`}>
      {buyLoading ? (
          ...
          <>
              ...
              <table className='w-full min-w-max table-auto text-left'>
                ...
                <tbody>
                  {formattedPurchaseData?.map((pool, index) => {
                    ...
                    //     <ClaimModal index={index} isMobile={false} successFlag={successFlag} setSuccessFlag={setSuccessFlag} pool={pool} tokenImage={tokenImage} claimableHistory={claimableHistory} poolInfo={poolInfo} purchasedAmount={purchasedAmount} totalPurchasedAmount={totalPurchasedAmount}/>
                    //   </tr>
                    // );
                  })}
                </tbody>
            ...
            <div className='flex sm:hidden w-full h-full gap-3 flex-col items-center overflow-y-auto px-2'>
              {formattedPurchaseData?.map((pool, index) => {
                ...
                return (
                  <ClaimModal index={index} isMobile={true} successFlag={successFlag} setSuccessFlag={setSuccessFlag} pool={pool} tokenImage={tokenImage} claimableHistory={claimableHistory} poolInfo={poolInfo} purchasedAmount={purchasedAmount} totalPurchasedAmount={totalPurchasedAmount}/>
                );
              })}
            </div>
...
export default TxHistoryBuyContainer