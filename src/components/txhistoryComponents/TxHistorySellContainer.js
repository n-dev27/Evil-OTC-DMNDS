...
function TxHistorySellContainer () {
  ...
  return (
    <>
    {isConnected ?
      <div className={`${sellLoading ? 'h-full' : 'h-full sm:h-fit'} md:text-large text-sm w-full flex flex-col justify-center items-center text-center`}>
      {sellLoading ? (
      ...
      <>
        <Card className='hidden sm:flex w-full max-h-[573px] overflow-y-auto rounded-xl bg-[rgba(255,255,255,0.05)]'>
          <table className='w-full min-w-max table-auto text-left'>
            ...
            <tbody>
              {formattedCreatedPoolsData?.map((pool, index) => {
        ...
        <div className='sm:hidden w-full h-[590px] gap-3 flex flex-col items-center overflow-y-auto px-2'>
              ...
              <div key={index} className='flex flex-col sm:hidden w-full h-fit rounded-lg bg-[rgba(255,255,255,0.05)] py-4 px-3 gap-4 border-[0.62px] border-[rgba(255,255,255,0.1)]'>
                <div className='w-full flex flex-col gap-3'>
                  ...
                  <div className='w-full flex flex-col gap-2'>
                    <p className='text-[rgba(100,116,139,1)] text-start text-xs font-[Inter]'>Amount Received</p>
                ...
                {soldValue[0].length !== 0 && clickDetail === true && clickDetailIndex === index && (
                  <div className='w-full bg-[rgba(255,255,255,0.05)] rounded-lg px-3 py-2 flex flex-col'>
                    {soldValue[0].map((item, index) => {
                        return (
                          <div key={index} className={`${index === 0 ? 'border-none' : 'border-t border-[rgba(255,255,255,0.1)]'} flex w-full flex-col py-3 gap-2`}>
                            <div className='w-full flex justify-between'>
                              <div className='flex gap-2 items-center'>
                                <p className='text-[rgba(100,116,139,1)] text-xs font-[Inter]'>To:</p>
                                <p className='text-[rgba(255,255,255,0.6)] text-xs font-[Inter]'>{item.buyer.slice(0, 6)}...{item.buyer.slice(item.buyer.length - 4, item.buyer.length)}</p>
                              </div>
                            </div>
                            <div className='flex gap-2'>
                              <div className='flex min-w-[68px] gap-1 items-center'>
                                <p className='text-[rgba(100,116,139,1)] text-xs font-[Inter]'>Sold:</p>
                                <div className='text-[rgba(255,255,255,0.6)] text-xs font-[Inter]'>{item.amount / 10 ** 18}</div>
                              </div>
                              <div className='flex gap-1 items-center'>
                                <p className='text-[rgba(100,116,139,1)] text-xs font-[Inter]'>Sell Price:</p>
                                <div className='text-[rgba(255,255,255,0.6)] text-xs font-[Inter]'>
                                  {pool?.poolData_4 === '0' ?  countLeadingZerosAfterDecimal(pool?.priceNative * (100 - pool?.poolData_3 / 10) / 100) + ' ETH | ' + countLeadingZerosAfterDecimal(pool?.priceUsd * (100 - pool?.poolData_3 / 10) / 100) + ' USD' :
                                  countLeadingZerosAfterDecimal(((utils.formatUnits(pool?.poolData_4?.toString(), pool?.tokenDecimals) * pool?.priceUsd / pool?.priceNative)))}</div>
                              </div>
                            </div>
                          </div>
                        )
...
export default TxHistorySellContainer